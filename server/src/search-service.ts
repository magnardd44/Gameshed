...
class SearchService {
	...
	extraFields = 'cover.url,aggregated_rating,screenshots.image_id,similar_games.name,first_release_date'
	...
	async get_game_and_extra(external_id: string) {
		...
		return await axios({
			...
			data: 'fields ' + this.gameFields + ',' + this.extraFields + '; where id=' + external_id + ';'
		})
		.then(response => {
			return{
				game_id: 0,
