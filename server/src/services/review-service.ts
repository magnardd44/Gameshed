...
class ReviewService {
  ...
  getGenre(genre_id: number) {
    return new Promise<Review[]>((resolve, reject) => {
      pool.query(
        `SELECT ge.genre_id, ge.genre_name, g.game_title, r.review_title, r.rating, r.text, COUNT(mr.relevant_id) AS likes
        FROM reviews r
        INNER JOIN games g ON g.game_id = r.game_id
        INNER JOIN mapping_relevant mr on r.review_id = mr.review_id
        ...
        GROUP BY ge.genre_id, ge.genre_name, g.game_title, r.review_title, r.rating, r.text
        ORDER BY likes DESC `,
        [genre_id],
        (error, results) => {
          if (error) return reject(error);
